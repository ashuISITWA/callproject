...
export default function MilfchatPageclient({ category, siteKey }: Props) {
  const t = useTranslations("weTested.milfCams");

  // Define the indices for the list items
  const discoverListItems = [1, 2, 3, 4, 5];
      ...
      <div className="w-full max-w-[1030px] mx-auto px-[15px]">
        ...
        <div className="flex flex-col gap-4 w-full max-w-[700px]">
          ...
          <ul className="text-[18px] text-black font-light list-disc pl-5">
            {discoverListItems.map((index) => (
              <li key={index}>{t(`discover_list_item_${index}`)}</li>
            ))}
          </ul>
          <h3 className="text-black text-[24px] md:text-[27px] font-bold">
            {t("hd_webcams_title")}
          ...
          <p className="text-[18px] text-black font-light">
            {t("hd_webcams_paragraph")}
          </p>
          <h3 className="text-black text-[24px] md:text-[27px] font-bold">
            {t("chat_247_title")}
          </h3>
          <p className="text-[18px] text-black font-light">
            {t("chat_247_paragraph")}
          </p>
        </div>
      </div>
    </section>
