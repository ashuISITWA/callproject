...
export async function generateMetadata({
  ...
  // Get SEO data from indexPage.randomChat.seo (messages/en.json)
  const localization = messages as any;
  const categorySeoData = localization.indexPage?.randomChat?.seo || {};
  ...
  Object.keys(messagesMap).forEach((loc) => {
    languages[loc] = loc === "en" ? "/randomchat" : `/${loc}/randomchat`;
  });

  // Generate canonical URL
  const canonical = locale === "en" ? "/randomchat" : `/${locale}/randomchat`;
  ...
  let pageUrl = `${BASE_URL}/randomchat`;
  if (pathname && pathname !== "/") {
    pageUrl = `${BASE_URL}${pathname}`;
  } else {
    pageUrl = locale === "en" ? `${BASE_URL}/randomchat` : `${BASE_URL}/${locale}/randomchat`;
  }

  // Replace {year} with current year in title
  const currentYear = new Date().getFullYear();
  const finalTitle = (categorySeoData.title || seoData.title || "Random Chat").replace("{year}", currentYear.toString());
  const finalOpenGraphTitle = (openGraphData.title || categorySeoData.title || seoData.title || "Random Chat").replace("{year}", currentYear.toString());
...
export default function Page() {
  return (
    <>
      ...
      <CamGrid category="random" siteKey="random"/>
      <SitesSlider category="random"/>
      <CategoryContent category="random" siteKey="random" />
      <Shortcuts />
    </>
  );
}
