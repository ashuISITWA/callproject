...
export async function generateMetadata({
  ...
  // Get SEO data from indexPage.ebonyChat.seo (messages/en.json)
  const localization = messages as any;
  const categorySeoData = localization.indexPage?.ebonyChat?.seo || {};
  ...
  Object.keys(messagesMap).forEach((loc) => {
    languages[loc] = loc === "en" ? "/ebonychat" : `/${loc}/ebonychat`;
  });

  // Generate canonical URL
  const canonical = locale === "en" ? "/ebonychat" : `/${locale}/ebonychat`;
  ...
  let pageUrl = `${BASE_URL}/ebonychat`;
  if (pathname && pathname !== "/") {
    pageUrl = `${BASE_URL}${pathname}`;
  } else {
    pageUrl = locale === "en" ? `${BASE_URL}/ebonychat` : `${BASE_URL}/${locale}/ebonychat`;
  }

  // Replace {year} with current year in title
  const currentYear = new Date().getFullYear();
  const finalTitle = (categorySeoData.title || seoData.title || "Ebony Chat").replace("{year}", currentYear.toString());
  const finalOpenGraphTitle = (openGraphData.title || categorySeoData.title || seoData.title || "Ebony Chat").replace("{year}", currentYear.toString());
...
export default function Page() {
  return (
    <>
      ...
      <CamGrid category="ebony" siteKey="ebonyChat"/>
      <SitesSlider category="ebony"/>
      <CategoryContent category="ebony" siteKey="ebonyChat" />
      <Shortcuts />
    </>
  );
}
